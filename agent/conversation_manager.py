...
class ConversationManager:
    ...
    async def create_conversation(self, title: str) -> dict:
        ...
        logger.info("Created conversation %s with title '%s'", conversation_id, title)
        return conversation
    ...
    async def list_conversations(self) -> list[dict]:
        ...
        for cid in conversation_ids:
            data = await self.redis.get(f"{CONVERSATION_PREFIX}{cid}")
            if data:
    ...
    async def chat(
        ...
        """
        logger.info("Chat request for conversation %s, stream=%s", conversation_id, stream)
    ...
    async def _save_conversation_messages(
        ...
        conversation["messages"] = [m.model_dump() for m in messages]
